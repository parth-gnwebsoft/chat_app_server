// Handlers/Logic/EditMessageHandler.cs
using Fleck;
...
{
    public class EditMessageHandler : IMessageHandler
    {
        ...
        public EditMessageHandler(IWebSocketStateService stateService, IMessageBroadcaster broadcaster, IMessageRepository messageRepository)
        {
        ...
        public Task HandleAsync(IWebSocketConnection socket, JsonElement payload)
        {
            ...
            try
            {
                ...
                if (isGroup)
                {
                    ...
                    if (groupId != null) { _broadcaster.BroadcastToGroup(groupId, "messageEdited", updatedMessage);}
                ...
                else
                {
                    ...
                    {
                        _broadcaster.Send(receiverSocket, "messageEdited", updatedMessage);
                    }
                    // Also send back to the sender
                    _broadcaster.Send(socket, "messageEdited", updatedMessage);
                }
            }
            catch (Exception ex)
            {
