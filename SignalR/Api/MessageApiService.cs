...
namespace Api
{
    public class MessageApiService : IMessageRepository
    {
        ...
        private readonly string _updateStatusApiUrl = $"https://api-demo.gnerp.in/api/Security/UserControllerUser/UpdateIsOnlineInChat";
        ...
        public async Task<ChatMessageResponse> SaveMessageAsync(ChatMessageRequest message, string authToken)
        {
            try
            {
                ...
                string jsonPayload = JsonSerializer.Serialize(message);
                 ...
                 Console.WriteLine($"[ChatMessageRequest] {jsonPayload}.");
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    savedMessage.messageUrls = message.messageUrls;
        ...
        public async Task UpdateUserOnlineStatusAsync(UpdateOnlineStatusRequest status, string authToken)
        {
            try
            {
                string jsonPayload = JsonSerializer.Serialize(status);
                var content = new StringContent(jsonPayload, Encoding.UTF8, "application/json-patch+json");

                var request = new HttpRequestMessage(HttpMethod.Put, _updateStatusApiUrl);
                request.Content = content;
                request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", authToken);
                ...
                if (response.IsSuccessStatusCode)
                {
                    Console.WriteLine($"[MessageApiService] User online status updated to {status.IsOnlineInChat}.");
                }
                else
                {
                    // Log failure but don't stop the connection/disconnection
                    string errorResponse = await response.Content.ReadAsStringAsync();
                    Console.WriteLine($"[MessageApiService] FAILED to update online status: {response.StatusCode} | Response: {errorResponse}");
                    // We don't throw here, as we still want the user to connect/disconnect
                }
            }
            catch (Exception ex)
            {
                // Log failure
                Console.WriteLine($"[MessageApiService] Exception in UpdateUserOnlineStatusAsync: {ex.Message}");
            }
        }
    }
}